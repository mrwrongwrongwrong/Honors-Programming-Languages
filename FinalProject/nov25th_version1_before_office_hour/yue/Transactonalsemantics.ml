...
let rec lookup element lst = match lst with 
    | [] -> raise Not_found
...
and start_operation (cd: cmds) ((stck, hp, position): state) = match cd with 
    ...
    | Cmd (c, cs) -> match c with (*c is a single command, cs represent multiple commands *)
        | Declare (v, c2) -> if v = (String.capitalize_ascii v) then (*4.3.2 *)
        ...
        | Call(e1, e2) -> (let temp_expr = eval_expr e1 (stck, hp, position) in (
                            ...
                            | Clo(v,cl,stck1) -> let stck2 = stck1 in push (Call([(v, position)], stck)) stck1; (*because push returns a unit type, hence use stck in *)
                                               let hp_new = add ((position,"val"), (eval_expr e2 (stck, hp, position))) hp in
                                               start_operation cs (exec_block cl (stck2, hp_new, position+1) )
                            | _ -> raise (Failure("Unable to call function:" ^ Printf.sprintf(e1))) )
        )
        ...
        | While (bexpr, c1) -> match bexpr with 
            | true -> (*do the next iteration *)
            | false -> (*start parse the next line of command *)
        )

and 
(*(stck,hp, position) has state type *)
exec_block cd (stck, hp, position) = 
...
and
eval_expr e (stck, hp, position) = match e with (*4.3.4 *)
    | Field e -> String e
    | One n -> Int n
    | Minus (e1, e2) -> let ee1 = eval_expr e1 (stck, hp, position) and ee2 = eval_expr e2 (stck, hp, position) in (
        match (ee1,ee2) with
        | (Int n1, Int n2) -> Int (n1 - n2)
        | _ -> raise(Failure ("The parsed operation is not allowed, since either"^ e1 ^ "or" ^ e2 ^ "is not Int type"))
    )
    | Null -> Null 
    | Var v -> (
        try lookup (eval_field v  )
    )
    | Dot (e1, e2) -> let l1 = eval_expr e1 (stck, hp, position) and let f1 = eval_expr e2 (stck, hp, positon) in (
        try lookup (eval_expr e1 e2 (stck, hp, position)) hp with 
        | Not_found -> raise (Failure ())
    )
    | Procedure (v, c2) -> Clo (v, c2, stck)