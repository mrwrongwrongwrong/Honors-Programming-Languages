...
(*cd has type cmds *)
let rec transitional_semantics cd = start_operation cd ([], [], 0) (*the first [] is stack, second [] is heap, 0 represents the location *) (*([], [], 0) is the initialization for a configuration of a command *)
...
and start_operation (cd: cmd) ((stck, hp, position): state) = match cd with 
    | VarDecl (v, c2) -> if v = (String.capitalize_ascii v) then (*4.3.2 *) (*this v corresponds to the variable x in the note *)
                            ...
                            let stck2 = push (var_position_to_frame_decl v position) stck in let hp_new = add ((position, "val"), Val (Loc Null)) hp in
    ...
    | Malloc v -> let hp1 = add (((stack_lookup v stck), "val"), int_to_tva position) hp in (stck, hp1, position+1)  (*was Val(Loc (position)) *)
    ...
    | Parallel (c1, c2) -> start_operation c2 (start_operation c1 (stck, hp, position))
    ...
    | Print e -> let stuff_to_print = ((string_of_expression e) ^ (string_of_tval (eval_expr e (stck, hp, position)))) in print_string(stuff_to_print ^"\n"); start_operation cd (stck,hp, position)
...
exec_block (cd:cmd) (stck, hp, position) = 
    let (stck_new, hp_new, position_new) = start_operation cd (stck, hp, position) in (
        ...
        let top_frame = top stck_new in match top_frame with
            ...
            | Decl v -> let stck_new = remove_top_frame stck_new in (stck_new, hp_new, position_new) (*be careful with this implementation *)
    )
